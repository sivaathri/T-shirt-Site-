...
const StackingSection = ({ title, image, reversed, bgClass, subTitle, highlight, textColor = "text-white", zIndex, showOverlay = true, grayscale = "grayscale-[0.3]" }) => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 z-0">
        <img 
          src={image} 
          className={`w-full h-full object-cover ${grayscale} hover:grayscale-0 transition-all duration-1000 opacity-100`}
          alt={title} 
        />
        {showOverlay && (
      ...
      <div className={`relative z-10 max-w-7xl mx-auto px-6 lg:px-12 w-full flex flex-col ${reversed ? 'items-end text-right' : 'items-start text-left'}`}>
        ...
        >
          ...
          <p className={`max-w-md ${isDark ? 'text-gray-300' : 'text-gray-700'} font-sans tracking-widest text-sm md:text-lg mb-8 uppercase`}>
            {subTitle}
          </p>
          <button className={`px-10 py-4 border ${isDark ? 'border-white text-white hover:bg-white hover:text-black' : 'border-black text-black hover:bg-black hover:text-white'} font-bold tracking-widest text-xs transition-all duration-500 rounded-full`}>
            EXPLORE COLLECTION
          </button>
        </motion.div>
      </div>
      ...
      <div className={`absolute top-0 left-0 w-full h-[1px] ${isDark ? 'bg-white/10' : 'bg-black/10'} z-20`}></div>
    </section>
  );
};
